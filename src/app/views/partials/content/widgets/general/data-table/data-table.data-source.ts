...
import { SuscribersService, SubscribersModel } from '../../../../../../core/_base/layout';
...
**/
export class DataTableDataSource implements DataSource<SubscribersModel> {
	// console.log(id);
	// Public properties
	 ...
	 */
	constructor(private dataTableService: SuscribersService) {
    ...
    loadItems(queryParams: QueryParamsModel) {
		...
		this.dataTableService.getSubscribers(this.id).pipe(
			tap(res => {
				console.log(res)
				const result = this.baseFilter(res['allSubscribers'], queryParams);
